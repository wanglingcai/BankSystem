...
public class DepositFrame extends JFrame {
    ...
    private JTextField amountField; // 存款金额输入框
    ...
    public DepositFrame(int userId) {
        ...
        // 添加存款金额输入项
        panel.add(new JLabel("存款金额（元）：", JLabel.RIGHT));
        ...
        // 确认存款按钮：触发存款逻辑
        JButton confirmBtn = UIUtils.createStyledButton("确认存款");
        confirmBtn.addActionListener(e -> doDeposit());
    ...
    private void doDeposit() {
        ...
        boolean success = service.deposit(userId, amount, password);
        if (success) {
            amountField.setText("");
